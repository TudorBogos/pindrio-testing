...
export class pindrioCheckoutPage {
  ...
  constructor(page: Page) {
    ...
    this.saveAddressButton = page.getByRole("checkbox", {
      name: "Save address in account",
    });
    this.savedCardSelector = page.getByRole("button", {
      name: "Card ending in 5098 Expires",
    });
  ...
  async proceedCheckout() {
    ...
    if (
      await this.page.getByRole("button", { name: "Send anyway" }).isVisible()
    ) {
