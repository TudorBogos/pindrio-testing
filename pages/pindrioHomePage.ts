...
export class pindrioHomePage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.logInSignUp = page.getByRole("button", {
      name: "Log in/Join IoTo club",
    });
    this.signUp = page.getByRole("button", {
      name: "Join IoTo club",
      exact: true,
    });
    this.logIn = page.getByRole("button", { name: "Log in", exact: true });
    this.btnAllProducts = page.getByRole("button", { name: "open menu" });
    this.btnElectroniceTab = page.getByRole("button", {
      name: "Electronice",
      exact: true,
    });
    this.btnSeeAllElectronice = page
  ...
  async logOut() {
    ...
    await this.loggedInButton.hover();
    await this.signOutButton.click();
    await expect(
      this.page.getByRole("button", { name: "Log in/Join IoTo club" })
    ).toBeVisible();
  }
