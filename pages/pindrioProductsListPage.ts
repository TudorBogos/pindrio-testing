...
export class pindrioProductsListPage {
  ...
  constructor(page: Page) {
    ...
    this.wishlistIcons = page
      .getByRole("button")
      .and(page.getByLabel("wishlist-icon"));
    this.addToCartButtons = page
      .locator(
        ".uppercase.flex.items-center.justify-center.font-semibold.border.rounded"
      )
      .filter({ hasNotText: "LOAD MORE" })
      .filter({ hasNotText: "SUBSCRIBE" })
      .and(page.getByRole("button"));
    this.itemsNames = page.locator(
      'p.w-full.h-\\[50px\\].mt-4.font-semibold[style*="display: -webkit-box"]'
    );
  }
